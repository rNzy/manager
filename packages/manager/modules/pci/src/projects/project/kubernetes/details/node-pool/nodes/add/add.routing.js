...
export default /* @ngInject */ ($stateProvider) => {
  $stateProvider.state('pci.projects.project.kubernetes.details.nodepools.details.nodes.add', {
    url: '/name',
    ...
    redirectTo: (transition) =>
      transition
        ...
        .then((flavor) => {
          if (!flavor) {
            return { state: 'pci.projects.project.kubernetes.details.nodepools.details.nodes' };
          }
          return false;
        }),
    params: {
    ...
    resolve: {
      cancelLink: /* @ngInject */ ($state, projectId, kubeId, nodePoolId) =>
        $state.href('pci.projects.project.kubernetes.details.nodepools.details', {
          projectId,
          kubeId,
          nodePoolId
        }),
      ...
      maxInstances: /* @ngInject */ (quotas, cluster) => {
        ...
        return get(quota, 'instance.maxInstances', 0) - get(quota, 'instance.usedInstances', 0);
      },
      ...
      breadcrumb: /* @ngInject */ ($translate) =>
        $translate.instant('kube_nodes_add'),
    },
  });
};
